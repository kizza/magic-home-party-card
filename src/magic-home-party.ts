...
export class MagicHomeParty extends LitElement {
  ...
  render() {
    ...
    const entitiesHtml = this.stateEntities.map(stateEntity =>
    ...
    return html`
      <ha-card @click=${this.playEffect}>
        <div class="gradient" style="color: ${foreground}; background: ${linearGradient(this.config.colours)}">
  ...
  private playEffect = () =>
    this.hass.callService('magic_home', 'set_custom_effect', {
      entity_id: this.config.entities,
      colours: this.config.colours,
      transition: "gradual"
    });
