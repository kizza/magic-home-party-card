...
describe('magic home party', () => {
  ...
  describe('editor', () => {
    const getSelectedColours = () => cy.get('magic-home-party-palette').first().as('selectedColours');
    ...
    describe.only('selecting targets', () => {
      // const getSelectedTargets = () => cy.get('.mdc-chip-set.items').as('selectedTargets');
      const getSelectedTargets = () => cy.get('.mdc-chip-set').as('selectedTargets');
      const getTargetPicker = () => cy.get('magic-home-party-targets-picker').as('targetPicker');

      it('allows adding entity id', done => {
        cy.document()
          .addEventListener('config-changed', event => {
            console.log("Here is the config", event)
            expect(event.detail.config.colours).to.deep.eq([yellow]);
            done();
          })
          .setupCustomCard('magic-home-party-card-editor', {})
          .then(getTargetPicker)
          .then(getSelectedTargets)
          .then(() => {
            cy.get('@targetPicker').should('exist');
            // cy.get('@selectedTargets').find('.mdc-chip').should('not.exist');
          })
          // .then(() => {
